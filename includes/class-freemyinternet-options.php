...
class FreeMyInternet_Options {
	 ...
	 * are deleted anyway.
	 *
	 * **This plugin passes no `default`** -- `FreeMyInternet_Settings::register_settings()`
	 * passes `type` and `sanitize_callback` only -- so there is no such filter
	 * here today and the trap is not armed. The helper is written this way
	 * regardless, so that adding one later cannot quietly break the migration.
	 * Do not read the paragraph above as a description of this plugin's
	 * `register_setting()` call; it is the reason the shape is worth keeping.
	 *
